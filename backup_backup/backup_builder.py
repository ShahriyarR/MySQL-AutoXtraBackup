...
class BackupBuilderChecker:
    ...
    def general_command_builder(self) -> str:
        ...
        if self.mysql_options.get('mysql_socket'):
            ...
            args += " --host={} --port={}".format(self.mysql_options.get('mysql_host'),
                                                  self.mysql_options.get('mysql_port'))
        # Adding compression support for backup
        if self.compression_options.get('compress') and \
           self.compression_options.get('compress_chunk_size') and \
           self.compression_options.get('compress_threads'):
            args += " --compress={}" \
                    " --compress-chunk-size={}" \
                    " --compress-threads={}".format(self.compression_options.get('compress'),
        ...
        if self.encryption_options.get('encrypt'):
            args += " --encrypt={}" \
                    " --encrypt-threads={}" \
                    " --encrypt-chunk-size={}".format(self.encryption_options.get('encrypt'),
    ...
    def extract_decrypt_from_stream_backup(self,
        ...
        file_name = "{}/{}/inc_backup.stream".format(self.backup_options.get('inc_dir'), recent_inc_bck)
        ...
        if self.xbstream_options.get('stream') == 'xbstream':
            xbstream_command = "{} {}".format(self.xbstream_options.get('xbstream'),
    ...
    def stream_encrypt_compress_tar_checker(self) -> None:
        if (
            self.xbstream_options.get('stream') == 'tar' and
            (self.encryption_options.get('encrypt') or self.compression_options.get('compress'))
        ):
    ...
    def decrypter(self, recent_full_bck: str, xtrabackup_inc_cmd: str, recent_inc_bck: str = None) -> None:
        ...
        xbcrypt_command = "{} -d -k {} -a {}".format(self.encryption_options.get('xbcrypt'),
                                                     self.encryption_options.get('encrypt_key'),
                                                     self.encryption_options.get('encrypt'))
        xbcrypt_command_extra = " -i {}/{}/xtrabackup_checkpoints.xbcrypt -o {}/{}/xtrabackup_checkpoints"
        xbcrypt_command += xbcrypt_command_extra.format(self.backup_options.get('full_dir'),
                                                        recent_full_bck,
                                                        self.backup_options.get('full_dir'),
                                                        recent_full_bck)
