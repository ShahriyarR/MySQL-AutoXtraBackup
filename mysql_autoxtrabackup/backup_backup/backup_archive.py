        ...
        for i in file_list:
            if len(file_list) == 1 or i != max(file_list):
                ...
                if self.backup_archive_options.get('move_archive') and \
                    ...
                    dir_name = str(self.backup_archive_options.get('archive_dir')) + '/' + i + '_archive'
                    logger.info("move_archive enabled. Moving {} to {}".format(self.backup_options.get('backup_dir'),
                                                                               dir_name))
                    try:
                        shutil.copytree(str(self.backup_options.get('backup_dir')), dir_name)
                    ...
                    archive_file = str(self.backup_archive_options.get('archive_dir')) + '/' + i + '.tar.gz'
                    if status:
