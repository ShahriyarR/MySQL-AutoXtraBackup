...
class BackupPrepareBuilderChecker:
    ...
    def prepare_command_builder(self, full_backup: str,
        ...
        if incremental:
            xtrabackup_prepare_cmd += " --incremental-dir={}/{}".format(self.backup_options.get('inc_dir'),
                                                                        incremental)
        if apply_log_only:
        ...
        if self.backup_options.get('xtra_options'):
            xtrabackup_prepare_cmd += "  {}".format(self.backup_options.get('xtra_options'))

        # Checking of extra prepare options were passed:
        if self.backup_options.get('xtra_prepare_options'):
            xtrabackup_prepare_cmd += "  {}".format(self.backup_options.get('xtra_prepare_options'))
