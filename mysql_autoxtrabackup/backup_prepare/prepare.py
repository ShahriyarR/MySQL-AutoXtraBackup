...
from mysql_autoxtrabackup.backup_prepare.prepare_builder import BackupPrepareBuilderChecker
...
class Prepare:
    ...
    def __init__(self, config: str = path_config.config_path_file, dry_run: Optional[bool] = None,
        ...
        if self.prepare_options.backup_options.get('prepare_tool'):
            self.prepare_options.backup_options['backup_tool'] = str(self.prepare_options.backup_options.
                                                                     get('prepare_tool'))
    ...
    def run_prepare_command(self, base_dir: Optional[str],
        ...
        # Decrypt backup
        self.prepare_options.decrypt_backup(base_dir,
                                            actual_dir)

        # Decompress backup
        self.prepare_options.decompress_backup(base_dir,
                                               actual_dir)
    ...
    def prepare_with_tags(self) -> Optional[bool]:
        ...
        found_backups = BackupPrepareBuilderChecker.\
                        parse_backup_tags(backup_dir=str(self.prepare_options.backup_options.get('backup_dir')),
                                          tag_name=self.tag)
        recent_bck = helpers.get_latest_dir_name(str(self.prepare_options.backup_options.get('full_dir')))
        ...
        if found_backups[1] == 'Full':  # type: ignore
            if recent_bck:
                ...
                self.run_prepare_command(str(self.prepare_options.backup_options.get('full_dir')),
                                         recent_bck, backup_prepare_cmd)
            ...
            if not os.listdir(str(self.prepare_options.backup_options.get('inc_dir'))):
                ...
                if self.prepare_only_full_backup():
                    ...
                    list_of_dir = helpers.sorted_ls(str(self.prepare_options.backup_options.get('inc_dir')))
                    ...
                    for dir_ in list_of_dir[:index_num + 1]:
                        ...
                        if dir_ != found_backups[0]: # type: ignore
                            logger.info("Preparing inc backups in sequence. inc backup dir/name is {}".format(dir_))
                            apply_log_only = True

                        else:
                            logger.info("Preparing last incremental backup, inc backup dir/name is {}".format(dir_))

                            # Extracting/decrypting from streamed backup and extra checks goes here
                            backup_builder.extract_decrypt_from_stream_backup(recent_inc_bck=dir_, flag=True)
                        ...
                        self.run_prepare_command(str(self.prepare_options.backup_options.get('inc_dir')),
                                                 dir_, backup_prepare_cmd)
    ...
    def prepare_inc_full_backups(self) -> Union[None, bool, Exception]:
        ...
        if not os.listdir(str(self.prepare_options.backup_options.get('inc_dir'))):
            ...
            if self.prepare_only_full_backup():
                logger.info("Preparing Incs: ")
                list_of_dir = sorted(os.listdir(str(self.prepare_options.backup_options.get('inc_dir'))))
                for inc_backup_dir in list_of_dir:
                    apply_log_only = None
                    if inc_backup_dir != max(os.listdir(str(self.prepare_options.backup_options.get('inc_dir')))):
                        logger.info(
                    ...
                    self.run_prepare_command(str(self.prepare_options.backup_options.get('inc_dir')),
                                             inc_backup_dir, backup_prepare_cmd)
