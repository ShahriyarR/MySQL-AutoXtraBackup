...
for i in `seq 1 1000`; do
     ...
     ${CONN_STR} -e "create table sysbench_test_db.sb1(id int(11) NOT NULL, c char(120) NOT NULL DEFAULT '')" &
     ${CONN_STR} -e "insert into sysbench_test_db.sb1 select id, c from sysbench_test_db.sbtest1" &
...
#echo "create unique index ix on sb1 (id)"|./use db1
    ...
    ${CONN_STR} -e "create table sysbench_test_db.sb2(id int(11) NOT NULL, c char(120) NOT NULL DEFAULT '')" &
    ${CONN_STR} -e "insert into sysbench_test_db.sb2 select id, c from sysbench_test_db.sbtest1" &
