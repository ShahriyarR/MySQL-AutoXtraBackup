...
for i in `seq 1 1000`; do
     ...
     ${CONN_STR} -e "create table sysbench_test_db.sb1 as select id,c from sysbench_test_db.sbtest1 where id < 150000" > /dev/null 2>&1
...
#echo "create unique index ix on sb1 (id)"|./use db1
    ...
    ${CONN_STR} -e "create table sysbench_test_db.sb2 as select id,c from sysbench_test_db.sbtest1 where id < 150000" > /dev/null 2>&1
