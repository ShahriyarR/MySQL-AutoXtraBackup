...
class WrapperForBackupTest(Backup):
    ...
    def parallel_sleep_queries(basedir, sql, sock):
        ...
        bash_command = "{}/run_sql_queries.sh {} {} '{}'".format(
            dir_path, basedir, sock, sql)
        try:
            Popen(split(bash_command),
                            stdin=None,
                            stdout=None,
                            stderr=None)
    ...
    def run_ddl_test_sh(basedir, sock):
        ...
        dir_path = os.path.dirname(os.path.realpath(__file__))
        bash_command = "{}/call_ddl_test.sh {} {} {}".format(
            dir_path, dir_path, basedir, sock)
        try:
            Popen(split(bash_command),
                            stdin=None,
                            stdout=None,
    ...
    def run_temp_table_test_sh(basedir, sock):
        ...
        dir_path = os.path.dirname(os.path.realpath(__file__))
        bash_command = "{}/call_temp_table_test.sh {} {} {}".format(
            dir_path, dir_path, basedir, sock)
        try:
            Popen(split(bash_command),
                            stdin=None,
    ...
    def run_call_create_index_temp_sh(basedir, sock):
        ...
        logger.debug("Trying to run call_temp_table_test.sh")
        dir_path = os.path.dirname(os.path.realpath(__file__))
        bash_command = "{}/call_create_index_temp.sh {} {} {}".format(
            dir_path, dir_path, basedir, sock)
        try:
            Popen(split(bash_command),
                            stdin=None,
    ...
    def run_call_innodb_online_alter_encryption_sql_sh(basedir, sock):
        logger.debug(
            "Trying to run call_innodb_online_alter_encryption_sql.sh")
        dir_path = os.path.dirname(os.path.realpath(__file__))
        bash_command = "{}/call_innodb_online_alter_encryption_sql.sh {} {} {}".format(
            dir_path, basedir, sock, dir_path)
        status, output = getstatusoutput(bash_command)
        if status == 0:
            logger.debug(
                "Running call_innodb_online_alter_encryption_sql.sh - OK")
        else:
    ...
    def run_call_innodb_online_alter_encryption_alters_sh(basedir, sock):
        logger.debug(
            "Trying to run call_innodb_online_alter_encryption_alters.sh")
        dir_path = os.path.dirname(os.path.realpath(__file__))
        bash_command = "{}/call_innodb_online_alter_encryption_alters.sh {} {} {}".format(
            dir_path, basedir, sock, dir_path)
